...
#si las celdas expiradas no estan en el estado de sesion, se crea un arreglo vafio en mystate
if "expired_cells" not in mystate: mystate.expired_cells = []
# Si los puntos no estan guardados, se inicializan en 0
...
if "GameDetails" not in mystate: mystate.GameDetails = ['Easy', 6, 1, '']  # difficulty level, sec interval for autogen, total_cells_per_row_or_col, player name
    ...
    if what_to_do == 'create':
        ...
        if mystate.GameDetails[3] != '':       # record in leaderboard only if player name is provided
            if os.path.isfile(vpth + 'leaderboard.json'):
                ...
                sc0, sc1, sc2, sc3, sc4 = st.columns((2, 3, 3, 3, 4))
                rknt = 0
...
# Funcion que muestra el menu inicial y las instrucciones
def InitialPage():
    with st.sidebar:
...
#funcion que verifica que se haya presionado una celda
    ...
    with st.sidebar:
        ...
        if st.button(f"🕹️ New Game", use_container_width=True):
            ...
            if mystate.GameDetails[0] == 'Easy':
                ...
                mystate.GameDetails[2] = 2         # total_cells_per_row_or_col
